...
interface ChatControllerSwagger {
    ...
    @Operation(
        summary = "카테고리별 질문 추천",
        description = """
            채팅방에 카테고리 기반 질문을 추천합니다.

            **버전 분기 동작:**
            - 1.3.0 이상 (X-App-Version 헤더 필수): 카테고리 기반 질문 추천
            - 1.3.0 미만 또는 헤더 없음: 기존 랜덤 질문 추천 (레거시)
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(responseCode = "400", description = "카테고리 미선택 또는 잘못된 카테고리"),
